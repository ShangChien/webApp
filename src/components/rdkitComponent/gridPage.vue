...
<template>
  <div ref="outBox" class="b-2 rd-2 b-indigo-100 relative min-w-230px min-h-160px h-full box-border">
    <div class="flex-none absolute rd-2 z-2 top-0 menubg" :style="{ width: `${outBoxW}px` }">
      <div class="ma-1 title-grid">
        ...
        <div class="flex flex-nowrap justify-end">
          <NTag v-show="outBoxW > 700 ? true : false" :bordered="false" class=" tagbg  ">
            <span class="text-1.2em ">Total: {{ initArray.length }}</span>
            <span class="text-1.2em "> ( {{ pageSize }} / page)</span>
          </NTag>
          <NPopover placement="bottom-end" trigger="click" display-directive="show">
            <template #trigger>
              <div
                class="i-icon-park-twotone:database-download
                        text-3xl
                        c-indigo-300
                        inline-block
                        hover:c-teal-300"
            ...
            <template #default>
              <export-mol :mol-list="initArray" />
            </template>
          </NPopover>
          <NPopover placement="bottom-end" trigger="click">
            <template #trigger>
              <button
                class="i-fluent-table-settings-20-filled
                        text-3xl
                        c-indigo-300
                        inline-block
                        hover:c-teal-300
    ...
    <NScrollbar :style="{ maxHeight: `${props.maxH}vh` }">
