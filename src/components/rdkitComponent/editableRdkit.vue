...
const { qsmiles, smiles, atoms, bonds, labels } = JSON.parse(JSON.stringify(props))
...
watch(
  props,
  async (newVal) => {
    const { qsmiles, smiles, atoms, bonds, labels } = JSON.parse(JSON.stringify(newVal))
    highlightMap.value = {
      id: 0,
      qsmiles,
      smiles,
...
function domClick($event: any) {
  const itemList = $event.target.getAttribute('class').split(' ')[0].split('-')
  if (itemList[0] === 'atom') {
    // 如果点击atom
    // 改颜色
