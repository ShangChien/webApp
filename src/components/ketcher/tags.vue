...
import { NButton, NCheckbox, NInput, NPopover, NTag } from 'naive-ui'
import { createReusableTemplate, useElementBounding, useElementSize } from '@vueuse/core'
...
async function select_all_or_none() {
  const len = await Promise.resolve(tags.value.length)
  if (len !== 0)
...
async function reverse() {
  ...
  const arr = await Promise.resolve(tags.value)
...
<template>
  <NPopover
    trigger="manual" placement="bottom-start" width="trigger" raw class="rd-2 box-border bg-white"
    :show-arrow="false"
    :show="showPopover"
  ...
  >
    <template #trigger>
      <div class="flex flex-nowrap justify-around pr-1 pl-1 max-w-full w-full items-center">
        <div
          class="rd-2 m-0 p-0 h-22px w-28px bg-blue-200 cursor-pointer
        ...
        <div class="overflow-hidden flex-auto transition-height-210 ml-1">
          <div ref="out_label" class="flex items-center flex-nowrap gap-1">
            <NTag
              v-for="(item, i) in selected" :key="i" type="success" round size="small" :closable="true"
              @close="delateTag(item)"
            >
              {{ item }}
            </NTag>
            <span
              v-if="selected.length === 0" class="w-full text-gray-4 text-center cursor-pointer"
              @click="changeShowPopover"
            >
    ...
    <div class="flex flex-nowrap justify-begin max-w-full p-1 rd-2 box-border items-start relative">
      <DefineTag v-slot="{ label }">
        <NButton
          class="m-0.5" round strong secondary size="tiny"
      ...
      <div
        class="flex-none flex flex-nowrap justify-around items-center
        rd-1.5 bg-blue-100 h-24px hover:bg-blue-200 "
      >
        <NCheckbox
          class=" ml-1.5 mr-1 " size="small"
        ...
        />
        <div
          class="i-carbon-contrast m-1 ml-2 bg-indigo-400 cursor-pointer text-xl transition-210 hover:(bg-indigo-600)"
          :style="{ transform: `rotateY(${angle}deg)` }"
          @click="reverse"
        />
      </div>
      <div
        class="flex-auto overflow-hidden flex flex-col justify-start items-start
        box-border relative transition-height-210"
      >
        <div
          class="overflow-hidden box-border transition-height-210 max-w-full"
          :style="{ height: `${labels_h}px` }"
        >
          <div
            ref="el_label" class="flex "
            :class="[expend ? 'flex-wrap' : 'flex-nowrap']"
          >
            <ReuseTag v-for="(tag, i) in tags" :key="i" :label="tag" />
          </div>
        </div>
        <div class="max-w-full w-full box-border">
          <NInput
            v-show="expend" v-model:value="inputValue" clearable
            class="w-full"
            round
            size="tiny" placeholder="新增tags" @keyup.enter="pushTag"
          />
        </div>
      </div>
      <div
        v-if="showExpand" class="rd-2 ml-2 m-0 p-0 h-24px bg-blue-100
        flex-none flex justify-center items-center hover:bg-blue-200"
      >
        <div
