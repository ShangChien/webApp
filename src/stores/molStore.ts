...
export const useMolStore = defineStore('mol', {
  state: () => ({
    mols: [
      ...
      { id: 10, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3C1', labels: ['螺芴', '芳环结构'], type: 'ligand' },
      { id: 11, smiles: 'CC(C)C1=CC(C(C)C)=CC=C1', labels: ['苯环二异丙基', '芳环结构'], type: 'ligand' },
      { id: 12, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3S1', labels: ['二苯并噻吩', '芳环结构'], type: 'ligand' },
      { id: 13, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3N1', labels: ['咔唑', '芳环结构'], type: 'ligand' },
      { id: 14, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3O1', labels: ['二苯并呋喃', '芳环结构'], type: 'ligand' },
      { id: 15, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3[Si]1', labels: ['硅螺芴', '芳环结构'], type: 'ligand' },
      { id: 16, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3S1', labels: ['二苯并噻吩', '芳环结构'], type: 'core' },
      { id: 17, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3N1', labels: ['咔唑', '芳环结构'], type: 'core' },
      { id: 18, smiles: 'C1(C=CC=C2)=C2C(C=CC=C3)=C3O1', labels: ['二苯并呋喃', '芳环结构'], type: 'core' },
    ] as molData[],
    nextId: 19,
  }),
  getters: {
    ...
    getByTypesAndLabels(state) {
      return (types: string[] = [], labels: string[] = []) => state.mols.filter(
        mol => types.includes(mol.type) || labels.some(item => mol?.labels.includes(item)),
      )
    },
